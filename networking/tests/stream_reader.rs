...
use failure::Error;
use networking::p2p::stream::{EncryptedMessageReaderBase, MessageReaderBase, StreamError};
...
async fn can_read_chunks() -> Result<(), Error> {
    ...
    let mut reader = MessageReaderBase { stream: mock };
    for data in chunks {
        let chunk = reader.read_message().await.expect("Should succeed");
...
#[async_std::test]
async fn cant_read_message_block_header_too_small_chunks() -> Result<(), Error> {
    let crypto_mock = CryptoMock::new();
    ...
    let message = block_header_message_encoded(1024 * 2);
    ...
    assert!(matches!(err, StreamError::ChunkSizeError { .. }));
