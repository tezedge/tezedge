...
impl Peer {
    ...
    {
        let props = Props::new_args::<Peer, _>((network_channel, local_node_info, tokio_executor, *socket_address));
        let actor_id = ACTOR_ID_GENERATOR.fetch_add(1, Ordering::SeqCst);
        sys.actor_of_props(&format!("peer-{}", actor_id), props)
    }
}

impl ActorFactoryArgs<(NetworkChannelRef, Arc<LocalPeerInfo>, Handle, SocketAddr)> for Peer {
    fn create_args((event_channel, info, tokio_executor, socket_address): (NetworkChannelRef, Arc<LocalPeerInfo>, Handle, SocketAddr)) -> Self {
        Peer {
...
impl Receive<Bootstrap> for Peer {
    ...
    fn receive(&mut self, ctx: &Context<Self::Msg>, msg: Bootstrap, _sender: Sender) {
        ...
        self.tokio_executor.spawn(async move {
            ...
            debug!(system.log(), "Bootstrapping"; "incoming" => incoming, "ip" => &peer_address, "peer" => myself.name(), "peer_uri" => myself.uri().to_string());
            match bootstrap(msg, info, &system.log()).await {
                ...
                Err(err) => {
                    warn!(system.log(), "Connection to peer failed"; "incoming" => incoming, "reason" => format!("{}", &err), "ip" => &peer_address, "peer" => myself.name(), "peer_uri" => myself.uri().to_string());
