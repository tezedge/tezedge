...
#[actor(BootstrapOutput, SendMessage)]
pub struct Peer {
...
impl Peer {
    ...
    {
        let props = Props::new_args::<Peer, _>((network_channel, tokio_executor, *socket_address));
        let actor_id = ACTOR_ID_GENERATOR.fetch_add(1, Ordering::SeqCst);
        sys.actor_of_props(&format!("peer-{}", actor_id), props)
    }
}

impl ActorFactoryArgs<(NetworkChannelRef, Handle, SocketAddr)> for Peer {
    fn create_args((event_channel, tokio_executor, socket_address): (NetworkChannelRef, Handle, SocketAddr)) -> Self {
        Peer {
...
impl Receive<BootstrapOutput> for Peer {
    type Msg = PeerMsg;

    fn receive(&mut self, ctx: &Context<Self::Msg>, msg: BootstrapOutput, _sender: Sender) {
        let BootstrapOutput(rx, tx, peer_public_key_hash, peer_id_marker, peer_metadata, peer_compatible_network_version, peer_address) = msg;

        let myself = ctx.myself();
        let system = ctx.system.clone();
        let mut net = self.net.clone();
        let network_channel = self.network_channel.clone();
        self.remote_addr = peer_address;
...
async fn begin_process_incoming(rx: Arc<Mutex<Option<EncryptedMessageReader>>>, net: Network, myself: PeerRef, event_channel: NetworkChannelRef, log: Logger) {
