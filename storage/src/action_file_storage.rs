...
impl ActionFileStorage {
    ...
    fn flush_entries_to_file(&mut self, block_hash: BlockHash, block_level: u32) -> Result<(), StorageError>{
        ...
        let actions = self.staging.remove(&block_hash).ok_or(
            ActionRecordError::MissingActions{hash : block_hash.to_base58_check()}
        )?;
    ...
    fn record(&mut self, context_action_message: &ContextActionMessage) -> std::result::Result<(), StorageError> {
        match &context_action_message.action {
            ...
            | ContextAction::Fold { block_hash: Some(block_hash), ..  } => {
                self.store_single_message(BlockHash::try_from(block_hash.clone())?, context_action_message);
                Ok(())
            }
            ContextAction::Commit { block_hash: Some(block_hash), .. } => {
                self.store_commit_message(BlockHash::try_from(block_hash.clone())?, &context_action_message)
            }
            _ => {Ok(())}
        }
    }
