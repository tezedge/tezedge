use std::{cell::RefCell, collections::HashMap, path::PathBuf, sync::RwLock, unimplemented};
...
pub struct InMemoryStorage<S: KeyValueSchema> {
    in_memory_data: RwLock<RefCell<HashMap<S::Key, S::Value>>>,
    path: PathBuf,
}
    ...
    S::Value: Codec,
{
    pub fn new(path: PathBuf) -> InMemoryStorage<S> {
        let storage: InMemoryStorage<S> = InMemoryStorage {
            in_memory_data: RwLock::new(RefCell::new(HashMap::new())),
            path: path.clone(),
        };
...
impl<S: KeyValueSchema> Drop for InMemoryStorage<S> {
    fn drop(&mut self) {
        ...
        for (k, v) in self.in_memory_data.read().unwrap().borrow().iter() {
