...
impl ContextRamStorage {
    ...
    pub fn get(&self, level: usize) -> Result<Option<ContextMap>, failure::Error> {
        ...
        if let Some(_) = self.context_blocks.get(level) {
            // do nothing, we are just validating
            {}
        } else {
            return Ok(None)
        }
        // take a slice from the begining to the desired block
        let relevant_blocks = &self.context_blocks[0..level + 1];
