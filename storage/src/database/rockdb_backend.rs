use crate::initializer::{initialize_rocksdb, RocksDbConfig, RocksDbColumnFactory};
use std::sync::Arc;
use rocksdb::{Cache, DB, ColumnFamilyDescriptor, WriteOptions, WriteBatch};
...
impl RocksDBBackend {
    pub fn new<C: RocksDbColumnFactory>(cache: &Cache, config: &RocksDbConfig<C>) -> Result<Self, Error> {
        let db = Self::open_kv(
            &config.db_path,
...
impl TezedgeDatabaseBackendStore for RocksDBBackend {
    fn put(&self, column: &'static str, key: Vec<u8>, value: Vec<u8>) -> Result<(), Error> {
        let cf = self.db.cf_handle(column).ok_or(Error::MissingColumnFamily {
            name: column
        })?;

        self.db.put_cf_opt(cf, &key, &value, &default_write_options())
            .map_err(Error::from)
    }
    ...
    fn delete(&self, column: &'static str, key: Vec<u8>) -> Result<(), Error> {
        ...
        self.db.delete_cf_opt(cf, &key, &default_write_options())
            .map_err(Error::from)
    }
    ...
    fn merge(&self, column: &'static str, key: Vec<u8>, value: Vec<u8>) -> Result<(), Error> {
        ...
        self.db.merge_cf_opt(cf, &key, &value, &default_write_options())
            .map_err(Error::from)
    }
    ...
    fn find(&self, column: &'static str, mode: BackendIteratorMode, limit: Option<usize>, filter: Box<dyn Fn((&[u8], &[u8])) -> Result<bool,SchemaError>>) -> Result<Vec<(Box<[u8]>, Box<[u8]>)>, Error> {
        ...
        let rocks_db_iterator = match mode {
            ...
            BackendIteratorMode::From(key, direction) => {
                self.db.iterator_cf(
                    cf,
                    rocksdb::IteratorMode::From(&key, direction.into()),
                )
            }
        };
    ...
    fn find_by_prefix(&self, column: &'static str, key: &Vec<u8>, _: usize, filter: Box<dyn Fn((&[u8], &[u8])) -> Result<bool,SchemaError>>) -> Result<Vec<(Box<[u8]>, Box<[u8]>)>, Error> {
        let cf = self.db.cf_handle(column).ok_or(Error::MissingColumnFamily {
            name: column
        })?;
