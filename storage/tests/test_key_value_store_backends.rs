...
fn get_storage(backend: &str, db_name: &str, cache: &Cache) -> Box<MerkleStorageKV> {
    match backend {
        "rocksdb" => Box::new(RocksDBBackend::new(
            Arc::new(get_db(db_name, &cache)),
        )),
        "sled" => {
            Box::new(SledBackend::new(sled::Config::new().path(get_db_name(db_name)).open().unwrap()))
        }
...
fn test_put_twice(backend: &str) {
    ...
    storage.put(&entry_hash(&[1]), &blob_serialized(vec![11])).unwrap();
    storage.put(&entry_hash(&[1]), &blob_serialized(vec![22])).unwrap();
    assert_eq!(blob_serialized(vec![22]), storage.get(&entry_hash(&[1])).unwrap().unwrap());
