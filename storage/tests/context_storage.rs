...
fn context_get_values_by_block_hash() -> Result<(), Error> {
    ...
    let value_2_1 = ContextRecordValue { action: ContextAction::Get { key: vec!("nice".to_string(), "to meet you".to_string()), operation_hash: None, block_hash: Some(str_block_hash_2.into()), context_hash: None, start_time: 0.0, end_time: 0.0 } };
    ...
    storage.put(&block_hash_2, &value_2_1)?;
...
fn context_get_values_by_contract_address() -> Result<(), Error> {
    ...
    let mut storage = ContextStorage::new(tmp_storage.storage());
    storage.put(&block_hash, &value)?;

    // block hash 1
    let values = storage.get_by_contract_address(&hex::decode("000003cb7d7842406496fc07288635562bfd17e176c4")?)?;
