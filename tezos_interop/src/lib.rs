...
mod tests {
    ...
    fn can_call_ocaml_fn_encode_operation_success() {
        ...
        let ocaml_future = runtime::run(move || {
            let f = ocaml::named_value("encode_operation").expect("function not registered");
            let s = f.call_exn(ocaml::Str::from(input_value)).unwrap();
            let s = ocaml::Str::from(s.clone());
            s.as_str().to_string()
        });
    ...
    fn can_call_ocaml_fn_encode_operation_error() {
        ...
        let ocaml_future = runtime::run(move || {
            let f = ocaml::named_value("encode_operation").expect("function not registered");
            match f.call_exn(ocaml::Str::from(input_value)) {
                Ok(ref value) => Ok(ocaml::Str::from(value.clone()).as_str().to_string()),
                Err(err) => Err(err)
            }
        });
        let ocaml_result = futures::executor::block_on(ocaml_future);
