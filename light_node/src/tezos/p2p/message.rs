...
pub enum P2pMessage {
    ...
    GetCurrentBranchMessage(GetCurrentBranchMessage),
}

/// Indicates that type has it's own ser/de schema.
///
/// Typically used together with [`BinaryMessage`].
pub trait HasEncoding {
    fn encoding() -> Encoding;
}

/// Trait for binary message to implement.
///
/// Binary messages could be written by a [`MessageWriter`](super::stream::MessageWriter).
/// To read binary message use  [`MessageReader`](super::stream::MessageReader).
...
/// Represents binary raw message received from peer node.
///
/// Difference from [`BinaryMessage`] is that it also contains [`MESSAGE_LENGTH_FIELD_SIZE`] bytes
/// of information about how many bytes is the actual message.
///
/// Example:
/// ```rust
...
/// Trait for json message to implement.
...
impl From<&super::client::Version> for Version {
    fn from(version: &super::client::Version) -> Self {
        Version {
...
impl BinaryMessage for AckMessage {

    fn as_bytes(&self) -> Result<Vec<u8>, ser::Error> {
        match self {
            &AckMessage::Ack => Ok(vec![0x00]),
            &AckMessage::Nack => Ok(vec![0xFF]),
        }
    }

    fn from_bytes(buf: Vec<u8>) -> Result<AckMessage, de::Error> {
        let mut buf = Cursor::new(buf).into_buf();
        let ack_byte = buf.get_u8();
        match ack_byte {
            0x00 => Ok(AckMessage::Ack),
            0xFF => Ok(AckMessage::Nack),
            _ => Err(de::Error::unsupported_value(&ack_byte))
        }
    }
}
...
mod tests {
    ...
    fn can_deserialize_current_branch_message() {
        ...
        match message {
            ...
            _ => panic!("Unsupported message: {:?}", message)
        }
    }
