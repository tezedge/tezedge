...
fn main() {
    ...
    let proc = Command::new("git").args(&["show", "-s", "--format=%ci", "HEAD"]).output();
    if let Ok(output) = proc {
        let git_commit_date = String::from_utf8(output.stdout).expect("Got non utf-8 response from `git show -s --format=%ci HEAD`");
