...
use shell::tezedge_state_manager::proposer_messages::{InjectBlock, ProposerMsg};
...
pub async fn inject_operation(
    ...
    if mempool_prevalidator_caller
        .try_tell(MempoolPrevalidatorMsg::MempoolOperationReceived(
...
pub async fn inject_block(
    ...
    env.proposer()
        .notify(ProposerMsg::InjectBlock(
            InjectBlock {
        ...
        .map_err(|e| RpcServiceError::UnexpectedError {
            reason: format!("Failed to inject operation, reason: {}", e),
        })?;
...
pub fn request_operations(env: &RpcServiceEnvironment) -> Result<(), RpcServiceError> {
    ...
    env.proposer()
        .notify(ProposerMsg::RequestCurrentHead)
        .map_err(|e| RpcServiceError::UnexpectedError {
            reason: format!("{}", e),
        })
}
