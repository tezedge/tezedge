...
//! This test generate P2P message structures of various sizes, aiming at
//! hitting enconding boundaries, like list lenght and dynamic and bounded
//! encoding sizes. Currently the following table represent this test coverage
//! (as measured by catching size-related errors reported by encoding
//! operation): [limits_coverage.md](limits_coverage.md).
//!
//! This coverage report can be generated by running the following test:
//! ```
//! cargo test -p tezos_messages --test encoding_diff_fuzz -- --ignored --nocapture limits_coverage
//! ```
...
fn test_message_with_factory<M: 'static + HasEncoding + BinWriter + Generated, F: TestFeedback>(
...
fn test_peer_message_with_factory<
    M: 'static + HasEncoding + BinWriter + Generated + Into<PeerMessage>,
    F: TestFeedback,
>(
    factory: &mut FocusedGeneratorFactory,
    feedback: &mut F,
...
fn test_message<M: 'static + HasEncoding + BinWriter + Generated>() {
    test_message_with_feedback::<M, _>(&mut EmptyFeedback)
}

fn test_peer_message<M: 'static + HasEncoding + BinWriter + Generated + Into<PeerMessage>>() {
    test_peer_message_with_feedback::<M, _>(&mut EmptyFeedback)
}

fn test_message_with_feedback<M: 'static + HasEncoding + BinWriter + Generated, F: TestFeedback>(
    feedback: &mut F,
) {
