...
impl SignaturePublicKey {
    ...
    pub fn from_tagged_bytes(pk: Vec<u8>) -> Result<SignaturePublicKey, ConversionError> {
        if pk.len() == 33 || pk.len() == 34 {
            ...
            match tag {
                0 => {
                    Self::from_hex_hash_and_curve(&hex::encode(&pk[1..]), "ed25519")
                }
                1 => {
                    Self::from_hex_hash_and_curve(&hex::encode(&pk[1..]), "secp256k1")
                }
                2 => {
                    Self::from_hex_hash_and_curve(&hex::encode(&pk[1..]), "p256")
                }
