...
fn connection_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("connection_decode_serde", |b| {
        b.iter(|| {
            let _ = <ConnectionMessage as tezos_messages::p2p::binary_message::BinaryMessageSerde>::from_bytes(&data).unwrap();
        });
    });
    c.bench_function("connection_decode_nom", |b| {
        b.iter(|| {
            let _ = <ConnectionMessage as tezos_messages::p2p::binary_message::BinaryMessageNom>::from_bytes(&data).unwrap();
        });
    });
    c.bench_function("connection_decode_raw", |b| {
        b.iter(|| {
            let _ = <ConnectionMessage as tezos_messages::p2p::binary_message::BinaryMessageRaw>::from_bytes(&data).unwrap();
        });
    });
}
