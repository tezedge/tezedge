...
use tezos_timing::{
    ...
    BlockMemoryUsage, ChannelError, Query, QueryKind, TimingMessage, TIMING_CHANNEL,
};
...
fn get_query_kind(name: &[u8]) -> Option<QueryKind> {
    ...
    // It's one of "mem", "mem_tree", "find", "find_tree"
    // "add", "add_tree" and "remove"
    match first {
        b'm' => {
            if name.len() == 3 {
                Some(QueryKind::Mem)
