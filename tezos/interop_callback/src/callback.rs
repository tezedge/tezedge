...
fn context_set(context_hash: Option<ContextHash>, block_hash: Option<BlockHash>, operation_hash: Option<OperationHash>, key: ContextKey, value: ContextValue) {
    context_send(ContextAction::Set { context_hash, block_hash, operation_hash, key, value }).unwrap();
}

fn context_delete(context_hash: Option<ContextHash>, block_hash: Option<BlockHash>, operation_hash: Option<OperationHash>, key: ContextKey) {
    context_send(ContextAction::Delete { context_hash, block_hash, operation_hash, key }).unwrap();
}

fn context_remove_rec(context_hash: Option<ContextHash>, block_hash: Option<BlockHash>, operation_hash: Option<OperationHash>, key: ContextKey) {
    context_send(ContextAction::RemoveRecord { context_hash, block_hash, operation_hash, key }).unwrap();
}

fn context_copy(context_hash: Option<ContextHash>, block_hash: Option<BlockHash>, operation_hash: Option<OperationHash>, from_key: ContextKey, to_key: ContextKey) {
    context_send(ContextAction::Copy { context_hash, block_hash, operation_hash, from_key, to_key }).unwrap();
}

fn context_checkout(context_hash: ContextHash) {
    context_send(ContextAction::Checkout { context_hash }).unwrap();
}

fn context_commit(parent_context_hash: Option<ContextHash>, block_hash: Option<BlockHash>, new_context_hash: ContextHash) {
