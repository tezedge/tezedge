...
impl BinaryReader {
    ...
    fn decode_record(&self, buf: &mut &[u8], schema: &[Field]) -> Result<Value, BinaryReaderError> {
    ...
    fn decode_value(
        ...
        match encoding {
            ...
            Encoding::Sized(sized_size, sized_encoding) => {
                let mut buf_slice = safe!(buf, *sized_size, {
                    let (slice, remaining) = buf.split_at(*sized_size);
                    *buf = remaining;
                    slice
                });
