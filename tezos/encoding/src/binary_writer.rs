...
fn encode_record(
    ...
    match value {
        ...
        _ => Err(Error::encoding_mismatch(&Encoding::Obj("", schema.to_vec()), value).into()),
    }
}
        ...
        Encoding::Tags(tag_sz, tag_map) => {
            match value {
                Value::Tag(ref tag_variant, ref tag_value) => {
                    let tag = tag_map.find_by_variant(tag_variant).ok_or_else(|| {
                        Error::custom(format!("No tag found for variant: {}", tag_variant))
                    })?;
                    let data_len_before_write = data.len();
                    // write tag id
                    ...
                    Ok(data
                        ...
                        .checked_sub(data_len_before_write)
                        .ok_or_else(|| {
                            Error::custom("Encoded message size overflow while encoding a tag")
                        })?)
                }
                Value::Enum(ref tag_variant, _) => {
                    ...
                    Ok(data
                        ...
                        .ok_or_else(|| {
                            Error::custom(
                                "Encoded message size overflow while encoding an enum value",
                            )
                        })?)
                }
                _ => Err(Error::encoding_mismatch(encoding, value).into()),
