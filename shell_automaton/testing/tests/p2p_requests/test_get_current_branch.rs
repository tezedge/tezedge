...
fn generate_chain(
    ...
    (1..=target_level).fold(vec![genesis_block.clone()], |mut chain, level| {
        let pred_hash = chain
            .last()
            .map(|b| b.hash.clone())
            .unwrap_or_else(|| genesis_block.hash.clone());
        let block_header = BlockHeaderBuilder::default()
            .level(level)
            .proto(1)
            .predecessor(pred_hash)
            .timestamp(10000000000 + (level as i64) * 30000)
            .validation_pass(4)
            .operations_hash(
            ...
            .build()
            .unwrap();
        chain.push(BlockHeaderWithHash::new(block_header).unwrap());
        chain
    })
}

fn build_expected_history(
