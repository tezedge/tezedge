    ...
    match &action.action {
        ...
        Action::PeerChunkWritePart(PeerChunkWritePartAction { address, written }) => {
            if let Some(peer) = state.peers.get_mut(address) {
                if let PeerWriteState::Writable { bytes_written, timestamp } = &mut peer.write_state {
                    if *bytes_written + *written >= state.config.quota.write_quota {
                        peer.write_state = PeerWriteState::OutOfQuota { timestamp: *timestamp };
                    } else {
                ...
                if let PeerChunkWriteState::Pending { chunk, written: chunk_written } = chunk_state {
