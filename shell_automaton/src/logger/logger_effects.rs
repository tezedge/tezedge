...
pub fn logger_effects<S: Service>(store: &mut Store<S>, action: &ActionWithMeta) {
    ...
    match &action.action {
        ...
        Action::PeerHandshakingFinish(content) => {
            ...
            "address" => content.address.to_string(),
            "public_key_hash" => slog::FnValue(|v| {
                match state.peers.get(&content.address) {
                    Some(peer) => match &peer.status {
                        PeerStatus::Handshaked(peer) => peer.public_key_hash.to_base58_check(),
                        _ => "peer not in expected state!".to_owned(),
                    }
                    None => "peer not found!".to_owned(),
                }
            }));
        }
        ...
        Action::PeerMessageReadSuccess(content) => {
            slog::debug!(log, "Received message from a peer";
                "address" => content.address.to_string(),
                "public_key_hash" => slog::FnValue(|v| {
                    match state.peers.get(&content.address) {
                        Some(peer) => match &peer.status {
        ...
        Action::PeerMessageWriteInit(content) => {
            slog::debug!(log, "Sending message to a peer";
                "address" => content.address.to_string(),
                "public_key_hash" => slog::FnValue(|_| {
                    match state.peers.get(&content.address) {
                        Some(peer) => match &peer.status {
                            PeerStatus::Handshaked(peer) => peer.public_key_hash.to_base58_check(),
                            _ => "peer not in expected state!".to_owned(),
