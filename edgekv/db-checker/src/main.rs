...
fn main() {
 let matches = App::new("Database CRC checker")
    ...
    let database_url = matches.value_of("DATABASE").expect("Provide database directory");
    ...
    println!("Read [{}] items, duration {} secs ~ {} ms", read_count, duration.as_secs(), duration.as_millis());
    println!("Reads per sec {}", read_count / duration.as_secs() as u128);
    println!("Avg read {} μs",  total_duration.as_micros() / read_count as u128);
}
