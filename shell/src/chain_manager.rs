...
use crate::tezedge_state_manager::proposer_messages::{
    InjectBlock, InjectBlockOneshotResultCallback, NetworkChannelMsg,
};
...
pub enum NotifyChainManagerMsg {
    NetworkEvent(NetworkEventNotificationRef),
    ShellCommand(ShellCommandNotificationRef),
    Shutdown,
}
...
#[actor(
    ...
    ShellCommandNotificationRef,
    ShellChannelMsg
)]
pub struct ChainManager {
    /// Proposer handle is a input to the tezedge state machine
    proposer: ProposerHandle,

    /// All events from shell will be published to this channel
    shell_channel: ShellChannelRef,
    shell_event_notifier: Arc<Notifier<ShellEventNotificationRef>>,
...
impl ChainManager {
    ...
    pub fn actor(
        ...
        proposer: ProposerHandle,
        shell_channel: ShellChannelRef,
        shell_event_notifier: Notifier<ShellEventNotificationRef>,
    ...
    ) -> Result<ChainManagerWrapper, CreateError> {
        let chain_manager_ref = sys.actor_of_props::<ChainManager>(
            ChainManager::name(),
            Props::new_args((
                block_applier,
                proposer,
                shell_channel,
                Arc::new(shell_event_notifier),
                persistent_storage,
        ...
        let (notify_chain_manager_tx, notify_chain_manager_rx) =
            mpsc::sync_channel(Self::NOTIFY_QUEUE_MAX_CAPACITY);

        // spawn mpsc queue receiver for notifications
        {
            std::thread::Builder::new()
                .name("chainman-notifier".to_owned())
                .spawn(move || {
                    run_notify_chain_manager_listener(
                        notify_chain_manager_rx,
                        chain_manager_ref,
                        log,
                    );
                })
        ...
        Ok(ChainManagerWrapper {
            notify_chain_manager_tx,
        })
    }
    ...
    fn check_mempool_completeness(&mut self, ctx: &Context<ChainManagerMsg>) {
        ...
        PeerState::schedule_missing_operations_for_mempool(proposer, peers, &ctx.system.log());
    }
    ...
    fn process_network_channel_message(
        ...
        match msg.as_ref() {
            NetworkEventNotification::PeerBootstrapped(peer_id, peer_metadata, _) => {
            ...
            NetworkEventNotification::PeerMessageReceived(received) => {
                                ...
                                if chain_state.get_chain_id().as_ref() == &message.chain_id {
                                        ...
                                        .as_ref()
                                    {
                                            ...
                                            block_storage.get(current_head_local.block_hash())?
                                        {
                                            ...
                                            tell_peer(proposer, &peer.peer_id, msg.into(), &log);
                                        }
                                    }
                                } else {
                            ...
                            PeerMessage::OperationsForBlocks(operations) => {
                                    ...
                                    )?
                                {
                                    ...
                                    if chain_state.process_block_operations_from_peer(
                                        ...
                                        // notify others that new all operations for block were received
                                        shell_event_notifier.notify(
                                            AllBlockOperationsReceived {
                            ...
                            PeerMessage::GetOperationsForBlocks(message) => {
                                for get_op in message.get_operations_for_blocks() {
                                    ...
                                    if let Some(op) = operations_storage.get(&key)? {
                                        tell_peer(proposer, &peer.peer_id, op.into(), &log);
                                    }
                                }
                            }
                                    ...
                                    .is_bootstrapped()
                                {
                                    ...
                                    match chain_state.can_accept_head(
                                        ...
                                        BlockAcceptanceResult::MutlipassValidationError(error) => {
                                            ...
                                            // blacklist peer
                                            proposer.notify(NetworkChannelMsg::BlacklistPeer(
                                                peer.peer_id.clone(),
                                                format!("{:?}", error),
                                            )).map_err(|e| format_err!("Failed to notify proposer for blacklist peer, reason: {}", e))?;
                                        }
                                    };
                                } else {
                            ...
                            PeerMessage::GetOperations(message) => {
                                ...
                                for operation_hash in requested_operations {
                                    ...
                                    if let Some(found) = mempool_storage.find(operation_hash)? {
                                        tell_peer(proposer, &peer.peer_id, found.into(), &log);
                                    }
                                }
                            }
    ...
    fn process_shell_command_notification(
        &mut self,
        ctx: &Context<ChainManagerMsg>,
        msg: ShellCommandNotificationRef,
    ) -> Result<(), Error> {
        match msg {
            ...
            ShellCommandNotification::AdvertiseToP2pNewCurrentHead(chain_id, block_hash) => {
                // get header and send it to p2p
                if let Some(header) = self.block_storage.get(&block_hash)? {
                    self.advertise_current_head_to_p2p(
                        &chain_id,
            ...
            ShellCommandNotification::AdvertiseToP2pNewCurrentBranch(chain_id, block_hash) => {
                // get header and send it to p2p
                if let Some(header) = self.block_storage.get(&block_hash)? {
                    self.advertise_current_branch_to_p2p(&chain_id, &header, &ctx.system.log())?;
                } else {
                    return Err(format_err!(
                        "BlockHeader ({}) was not found!",
                        block_hash.to_base58_check()
            ...
            ShellCommandNotification::InjectBlock(inject_block, result_callback) => {
                self.process_injected_block(inject_block, result_callback, ctx)?;
            }
            ShellCommandNotification::RequestCurrentHead => {
                let ChainManager {
                    peers,
                    chain_state,
                    proposer,
                ...
                peers.iter_mut().for_each(|(_, peer)| {
                    ...
                    tell_peer(proposer, &peer.peer_id, msg.clone(), &ctx.system.log())
                });
            }
            ShellCommandNotification::PeerBranchSynchronizationDone(msg) => {
    ...
    fn process_injected_block(
        ...
        if is_new_block {
            ...
            self.shell_event_notifier.notify(
                BlockReceived {
    ...
    fn advertise_current_head_to_p2p(
        ...
        self.peers.iter().for_each(|(_, peer)| {
            ...
            if can_send_msg {
                tell_peer(&self.proposer, &peer.peer_id, msg, log)
            }
        });
    }
...
impl
    ActorFactoryArgs<(
        ...
        ProposerHandle,
        ShellChannelRef,
        Arc<Notifier<ShellEventNotificationRef>>,
        PersistentStorage,
            ...
            .for_each(|(uri, state)| {
                ...
                let known_higher_head = match state.current_head_level {
                    Some(peer_level) => match self.current_head.has_any_higher_than(peer_level) {
                        Ok(result) => result,
                        Err(_) => {
                            warn!(log, "Failed to collect current local head";
                ...
                if should_disconnect {
                    if let Err(err) = self.proposer.notify(NetworkChannelMsg::PeerStalled(state.peer_id.clone())) {
                        warn!(log, "Failed to notify proposer (PeerStalled - chain_manager)"; "reason" => format!("{:?}", err));
                    }
                }
            });
    }
...
impl Receive<NetworkEventNotificationRef> for ChainManager {
    type Msg = ChainManagerMsg;
...
impl Receive<ShellCommandNotificationRef> for ChainManager {
    type Msg = ChainManagerMsg;
    ...
    fn receive(
        ...
        match self.process_shell_command_notification(ctx, msg) {
            Ok(_) => (),
            Err(e) => {
                warn!(ctx.system.log(), "Failed to process shell command notification"; "reason" => format!("{:?}", e))
            }
        }
    }
}
...
fn run_notify_chain_manager_listener(
    rx: mpsc::Receiver<NotifyChainManagerMsg>,
    ...
    loop {
        match rx.recv() {
            Ok(NotifyChainManagerMsg::NetworkEvent(notification)) => {
                // TODO: here, we just temporary send to chain_manager actor
                chain_manager_ref.tell(notification, None);
            }
            Ok(NotifyChainManagerMsg::ShellCommand(notification)) => {
                // TODO: here, we just temporary send to chain_manager actor
                chain_manager_ref.tell(notification, None);
            }
            Ok(NotifyChainManagerMsg::Shutdown) => {
                info!(log, "Chain manager notification listener finished");
                return;
            }
            Err(mpsc::RecvError) => {
                info!(
                    log,
                    "Chain manager notification listener finished (by disconnect)"
                );
                return;
            }
        }
...
pub struct ChainManagerWrapper {
    notify_chain_manager_tx: mpsc::SyncSender<NotifyChainManagerMsg>,
}

impl NotifyCallbackRegistrar<NetworkEventNotificationRef> for ChainManagerWrapper {
    fn subscribe(&self, notifier: &mut Notifier<NetworkEventNotificationRef>) {
        notifier.register(
            Box::new(ChainManagerNotifyCallback(
                self.notify_chain_manager_tx.clone(),
            )),
...
impl NotifyCallbackRegistrar<ShellCommandNotificationRef> for ChainManagerWrapper {
    fn subscribe(&self, notifier: &mut Notifier<ShellCommandNotificationRef>) {
        notifier.register(
            Box::new(ChainManagerNotifyCallback(
                self.notify_chain_manager_tx.clone(),
            )),
            "chain-manager-shell-command-listener".to_string(),
        );
    }
}

struct ChainManagerNotifyCallback(mpsc::SyncSender<NotifyChainManagerMsg>);

impl NotifyCallback<NetworkEventNotificationRef> for ChainManagerNotifyCallback {
    fn notify(
        &self,
        notification: NetworkEventNotificationRef,
    ) -> Result<(), SendNotificationError> {
        self.0
            .send(NotifyChainManagerMsg::NetworkEvent(notification))
    ...
    fn shutdown(&self) -> Result<(), ShutdownCallbackError> {
        self.0
            .try_send(NotifyChainManagerMsg::Shutdown)
...
impl NotifyCallback<ShellCommandNotificationRef> for ChainManagerNotifyCallback {
    fn notify(
        &self,
        notification: ShellCommandNotificationRef,
    ) -> Result<(), SendNotificationError> {
        self.0
            .send(NotifyChainManagerMsg::ShellCommand(notification))
            .map_err(|e| SendNotificationError {
                reason: format!("{}", e),
            })
    }
