...
fn test_process_bootstrapping_current_branch_on_level3_then_current_heads(
    ...
    assert!(
        node.mempool_prevalidator_factory.find_mempool_prevalidator_caller(&node.tezos_env.main_chain_id()?).is_none()
    );
    ...
    assert!(
        node.mempool_prevalidator_factory.find_mempool_prevalidator_caller(&node.tezos_env.main_chain_id()?).is_some()
    );
...
fn test_process_current_head_with_malformed_blocks_and_check_blacklist(
    ...
    let peers_mirror = Arc::new(RwLock::new((
        HashMap::new(),
        HashMap::new(),
        HashMap::new(),
    )));
    let network_event_listner = common::infra::test_actor::NetworkEventListener::create_and_start(
        peers_mirror.clone(),
        log.clone(),
    )?;
    ...
    assert!(mocked_peer_node
        ...
        .is_ok());
    common::infra::test_actor::NetworkEventListener::verify_connected(
        &mocked_peer_node,
        peers_mirror.clone(),
    )?;
    ...
    common::infra::test_actor::NetworkEventListener::verify_blacklisted(
        &mocked_peer_node,
        peers_mirror.clone(),
    )?;
    drop(mocked_peer_node);
